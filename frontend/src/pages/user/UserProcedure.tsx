...
const UserProcedure: React.FC = () => {
  ...
  const cancelProcedure = async (id: string) => {
    try {
      const res = await fetch(`${API_URL}/api/procedures/${id}/cancel`, {
        method: 'PUT',
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || 'Annulation impossible');
      }
                ...
                return (
                  <div key={procedure._id} className='p-6 hover:bg-gray-50 transition-colors'>
                    <div className='flex items-start justify-between'>
                      ...
                      <div>
                        {procedure.statut !== 'Terminée' && procedure.statut !== 'Annulée' && (
                          <button onClick={() => cancelProcedure(procedure._id)} className='px-3 py-1 text-sm rounded bg-red-600 text-white hover:bg-red-700'>
                            Annuler
                          </button>
                        )}
                      </div>
