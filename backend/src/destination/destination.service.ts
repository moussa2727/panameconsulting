...
export class DestinationService {
    ...
    async findAll(
        ...
        try {
            ...
            this.logger.error(`❌ Erreur récupération destinations: ${error.message}`);
    ...
    async findAllWithoutPagination(): Promise<Destination[]> {
        try {
            ...
            this.logger.error(`❌ Erreur récupération toutes destinations: ${error.message}`);
            throw new InternalServerErrorException('Erreur lors de la récupération des destinations');
        }
    }
    ...
    async findOne(id: string): Promise<Destination> {
        try {
            ...
            this.logger.error(`❌ Erreur récupération destination ${id}: ${error.message}`);
            throw new InternalServerErrorException('Erreur lors de la récupération de la destination');
        }
    }
    ...
    async findByCountry(country: string): Promise<Destination | null> {
        try {
            ...
            this.logger.error(`❌ Erreur recherche par pays ${country}: ${error.message}`);
            throw error;
        }
    }
    ...
    async update(
        ...
        try {
            ...
            const hasUpdateData = Object.keys(updateDestinationDto).length > 0 || imageFile;
            if (!hasUpdateData) {
            ...
            if (updateDestinationDto.country && 
                ...
                if (countryConflict) {
                    throw new ConflictException('Une destination avec ce nom existe déjà');
                }
            }
            ...
            if (!updatedDestination) {
                throw new NotFoundException(`Destination avec ID ${id} non trouvée après mise à jour`);
            }
            ...
            if (oldImagePath) {
                try {
                    ...
                    this.logger.warn(`⚠️ Impossible de supprimer l'ancienne image: ${cleanupError.message}`);
                }
            }

            this.logger.log(`✅ Destination mise à jour: ${updatedDestination.country}`);
            return updatedDestination;

        } catch (error) {
            this.logger.error(`❌ Erreur mise à jour destination ${id}: ${error.message}`);
            ...
            throw new InternalServerErrorException('Erreur lors de la mise à jour de la destination');
        }
    }
    ...
    async count(filters: any = {}): Promise<number> {
        try {
            ...
            throw new InternalServerErrorException('Erreur lors du comptage des destinations');
        }
    }
